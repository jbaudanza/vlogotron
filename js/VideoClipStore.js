...
const refs$ = currentRoute$.map(route => {
  if (route.uid) return refsForUids(route.uid);
  else return null;
});
...
function reduceClipIdsToPromises(ref, acc, clipIds) {
  ...
  forEach(clipIds, (clipId, note) => {
    if (!(clipId in next.exists)) {
      next.promises.push(
        mapClipIdToPromise(ref, clipId).then(result => ({ [note]: result }))
      );
...
function mapRefsToRemoteUrls(refs) {
  if (refs) {
    return Observable.fromEvent(refs.events.orderByKey(), "value")
...
const loaded$ = http$
  ...
  .switchMap(list => Observable.combineLatest(list.map(http => http.loaded)))
  .map(sum)
  .startWith(0);
...
const progress$ = Observable.combineLatest(
  loaded$,
  total$,
...
const progressStreams$ = loadingContext$.flatMap(x =>
  Observable.from(x.progressList)
);
  ...
  function reducer(acc, command) {
    ...
    if (command.play) {
      if (command.play in recording) {
        console.warn(
          "Received two play commands for the same note",
          command.play
        );
      } else {
        recording = Object.assign({}, recording, {
          [command.play]: audioContext.currentTime
        });
      }
    }
    ...
    if (command.pause) {
      if (command.pause in recording) {
        ...
        console.warn(
          "Received a pause command without a preceeding play command",
          command.pause
        );
      }
    }
...
export default class VideoClipStore {
  constructor(touchplayCommandsStreams$) {
    ...
    const livePlayCommands$ = Observable.merge(
    ...
    this
      .videoClips$ = Observable.combineLatest(
      localUrls,
      remoteUrls$,
...
function subscribeToAudioPlayback(playCommands$) {
  ...
  playCommands$.withLatestFrom(audioBuffers$).subscribe(([
    cmd,
    audioBuffers
  ]) => {
...
function startPlayback(song, startPosition, playUntil$) {
  ...
  const playCommandsForVisuals$ = Observable.from(
    flatten(
      truncatedSong.map(function(note) {
        ...
        function makeEvent(obj, when) {
          return Observable.of(Object.assign({ when }, obj)).delay(
            (when - audioContext.currentTime) * 1000
          );
        }

        return [
          makeEvent({ play: note[0] }, startAt),
          makeEvent({ pause: note[0] }, stopAt)
        ];
      })
    )
  )
    ...
    .subscribe({
      next([commands, audioBuffers]) {
        commands.forEach(command => {
          ...
          if (audioBuffer) {
            const startAt =
              playbackStartedAt + beatsToTimestamp(command[1], bpm);
  ...
  const position$ = Observable.of(0, animationFrame)
    .repeat()
    .map(() =>
