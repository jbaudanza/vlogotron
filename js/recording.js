...
export function start(mediaStream, finish$, abort$) {
  ...
  takeUntil$.subscribe({complete() { videoRecorder.stop() }});
  ...
  //
  const audioProcessEvent$ = audioProcessEventsFromMediaStream(mediaStream)
      .takeUntil(takeUntil$);
  ...
  const audioBuffer$ = Observable.race(finishedAudioBuffer$, abort$.ignoreElements());
